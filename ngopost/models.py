...
class NGOPost(models.Model):
    ...
    post_type = models.ForeignKey(PostTypeOption, on_delete=models.SET_NULL, null=True, blank=True)
    
    donation_frequency = models.CharField(max_length=20,  choices=DonationFrequency.choices,  default=DonationFrequency.ONETIME)
    target_donation = models.DecimalField(max_digits=12, decimal_places=2)
    donation_received = models.DecimalField(max_digits=12,  decimal_places=2,  default=0.00)

    country = models.ForeignKey(CountryOption, on_delete=models.SET_NULL, null=True, blank=True)
    state = models.ForeignKey(StateOption, on_delete=models.SET_NULL, null=True, blank=True)
    city = models.ForeignKey(CityOption, on_delete=models.SET_NULL, null=True, blank=True)
    pincode = models.CharField(max_length=10)
    age_group = models.ForeignKey(AgeOption, on_delete=models.SET_NULL, null=True, blank=True)
    gender = models.ForeignKey(GenderOption, on_delete=models.SET_NULL, null=True, blank=True)
    spending_power = models.ForeignKey(SpendingPowerOption, on_delete=models.SET_NULL, null=True, blank=True)
